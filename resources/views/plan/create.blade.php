    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <h1 class="text-center font-semibold text-xl text-gray-800 leading-tight">Introduzca los datos del plan
                </h1>
                <div class="flex justify-center ml-3 mt-6">
                <form action="{{ isset($planDelMes) ? route('plan.update', $planDelMes->id) : route('plan.store')}}" action="{{ route('plan.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @if (isset($planDelMes))
                           @method('PUT')
                        @endif
                        <div>
                            ...
                            <select class="select select-primary w-full max-w-xs" name="mes" id="mes">
                                <option  selected>Seleccione el mes correspondiente</option>
                        ...
                        <div class="mt-6 mr-10">
                            ...
                            <select class="select select-primary w-full max-w-xs" name="anno" id="anno">
                                ...
                                @for ($i = $anno; $i > $anno - 20; $i--)
                                    <option  value="{{$i}}" {{ old('anno', $plan->anno ?? '') == $i ? 'selected' : '' }}>{{$i}}</option>
                                @endfor
                            </select>
                        </div>
                        <div>
                            <div class="mt-6">
                                ...
                                <select class="select select-primary w-full max-w-xs" name="productos_id" id="productos_id" >
                                    <option selected >Seleccione el producto correspondiente</option>
                                    @foreach ($productos as $producto)
                                        <option   value="{{$producto->id}}" {{ old('anno', $plan->anno ?? '') == $i ? 'selected' : '' }}>{{$producto->nombre}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mt-6">
                                ...
                                value="{{old('cantidad', $plan->cantidad ?? '')}}"
                                    class="input input-bordered input-primary w-full max-w-xs" />
                            </div>
                            <div class="mt-6">
                                <label class="mr-6" for="precio">Precio</label>
                                <input type="number" name="precio" id="precio" required placeholder="Escribe aquí"
                        ...
                        <div class="flex justify-center mt-10">
                            ...
                            <a href="{{ route('plan.index') }}" class="btn btn-glass">Cancelar</a>
                        </div>
                    </form>
                </div>
